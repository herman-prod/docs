...
You're right, I shouldn't have copy-pasted that feedback directly. Let me reframe around your insight about action classification:
markdown---
title: Introduction
description: "AARM is an open system specification for securing AI-driven actions at runtime. Build systems that intercept, authorize, and audit autonomous actions before they execute."
---
...
AARM is not a product, library, or service you install. It is a specification that describes the components, behaviors, and conformance requirements for systems that secure AI agents.

<Note>
**The core insight**: Not all actions can be judged the same way. Some are always forbidden. Others are allowed by policy but should be blocked if context reveals misalignment with the user's actual intent. And some are denied by default but should be permitted when context confirms they're exactly what the user asked for. Security decisions depend on understanding *why* the agent is acting, not just *what* it's doing.
</Note>

You use AARM to **design and build** your own runtime security system, or to **evaluate** whether existing solutions meet the specification.
...
Existing tools can block, but they don't understand *intent*:

| Tool | Can Block? | The Gap |
|------|------------|---------|
| **SIEM** | Yes, after pattern detection | Sees "API call" not "agent is emailing customer data to external address after reading the database" |
| **API Gateways** | Yes, via rules | Checks identity and rate limits, not whether this action makes sense given what the agent just read |
| **Firewalls** | Yes, at perimeter | Agent is already inside with valid credentials |
| **Prompt Guardrails** | Yes, for text | Filters prompts, not actions—and attackers aren't stupid |
| **Human in the Loop** | Yes, manually | Doesn't scale, and can itself be exploited via social engineering |

<Card title="Full Problem Analysis" icon="magnifying-glass" href="/problem">
  Deep dive into the runtime security gap and why context-aware evaluation is required
</Card>
